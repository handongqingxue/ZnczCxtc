...
function showCompontByQx(){
	...
	if(yhm=="admin"){
		...
		var tjclQx='${requestScope.tjclQx}';
		var scclQx='${requestScope.scclQx}';
		var qxIdsArr=qxIds.split(",");
		for(var i=0;i<qxIdsArr.length;i++){
			if(qxIdsArr[i]==tjclQx){
				addLB.show();
			}
			if(qxIdsArr[i]==scclQx){
				removeLB.show();
			}
		}
	}
...
function showOptionButByQx(){
	if(yhm=="admin"){
		...
		var xgclQx='${requestScope.xgclQx}';
		var qxIdsArr=qxIds.split(",");
		for(var i=0;i<qxIdsArr.length;i++){
			if(qxIdsArr[i]==xgclQx){
				$(".tab1_div .edit_a").css("visibility","visible");
			}
		}
	}
	...
	tab1=$("#tab1").datagrid({
		...
		columns:[[
            ...
            {field:"id",title:"操作",width:150,formatter:function(value,row){
            	...
            	+"&nbsp;|&nbsp;<a class=\"edit_a\" href=\""+clglPath+"zhcx/edit?id="+value+"\">修改</a>";
            	return str;
            }}
	    ]],
